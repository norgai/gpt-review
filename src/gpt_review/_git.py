...
def _commit_message(gpt4: bool = False, large: bool = False) -> str:
    ...
    Args:
        gpt4 (bool, optional): Whether to use gpt. Defaults to False.
        large (bool, optional): Whether to use gpt-32k. Defaults to False.
...
def _commit(gpt4: bool = False, large: bool = False, push: bool = False) -> Dict[str, str]:
    ...
    Args:
        gpt4 (bool, optional): Whether to use gpt. Defaults to False.
        large (bool, optional): Whether to use gpt. Defaults to False.
...
class GitCommandGroup(GPTCommandGroup):
    ...
    def load_arguments(loader: CLICommandsLoader) -> None:
        with ArgumentsContext(loader, "git commit") as args:
            args.argument(
                "gpt4",
                help="Use gpt for generating commit messages instead of gpt-35-turbo.",
                default=False,
                action="store_true",
            )
            args.argument(
                "large",
                help="Use gpt model for generating commit messages.",
                default=False,
                action="store_true",
            )
            args.argument(
